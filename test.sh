...
while getopts "d123456789abc" opt; do
	case $opt in
		d)
			...
			DESC8="test8: ${VIOLET}(${BLACK}echo -nnn -----nn --nnnnn >outfile1):${NC}\\n"
		...
		6)
			...
			if diff -a ${FOLDER6}outfile3 ${FOLDER6}test3 > ${FOLDER6}diff ; then
				TEST6="${DESC5} ${GREEN}[OK]${NC}\\n"
			else 
				TEST6="${DESC5} ${RED}[nope]${NC}\\n"
			fi
			;;
		7)
			...
			if diff -a ${FOLDER7}outfile1 ${FOLDER7}test1 > ${FOLDER7}diff ; then
				TEST7="${DESC5} ${GREEN}[OK]${NC}\\n"
			else 
				TEST7="${DESC5} ${RED}[nope]${NC}\\n"
			fi
			;;
		8)
			rm -f ${FOLDER8}outfile1 ${FOLDER8}test1
			./minishell 8
			 echo -nnn -----nn --nnnnn >test1
